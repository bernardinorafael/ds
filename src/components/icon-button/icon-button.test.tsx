...
describe("IconButton", () => {
  ...
  it("should render with aria-label", () => {
    setup(<IconButton icon="trash-outline" aria-label="Delete" />)
    expect(screen.getByRole("button", { name: "Delete" })).toBeInTheDocument()
  })

  it("should forward ref to the button element", () => {
    const ref = createRef<HTMLButtonElement>()
    setup(<IconButton ref={ref} icon="trash-outline" aria-label="Delete" />)
  ...
  it("should be disabled when disabled prop is set", () => {
    setup(<IconButton icon="trash-outline" aria-label="Delete" disabled />)
    expect(screen.getByRole("button")).toBeDisabled()
  })

  it("should call onClick when clicked", async () => {
    const onClick = vi.fn()
    setup(<IconButton icon="trash-outline" aria-label="Delete" onClick={onClick} />)
  ...
  it("should render tooltip when tooltip prop is provided", async () => {
    setup(
      <TooltipProvider>
        <IconButton icon="trash-outline" aria-label="Delete" tooltip="Delete item" />
      </TooltipProvider>
    )
